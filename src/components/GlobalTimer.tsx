 ...
 * - DM mode: full controls (start/pause/reset, ±10s, presets)
 * - Player mode: read-only display (only shown when playerViewConfig.showTimer is true)
 *
...
import { Play, Pause, RotateCcw, Plus, Minus, Hourglass } from 'lucide-react';
import { useGameState } from '@/hooks/useGameState';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

interface GlobalTimerProps {
  /** When true, renders only the visual display (no controls). Used in PlayerView. */
  readOnly?: boolean;
}
...
export const GlobalTimer = ({ readOnly = false }: GlobalTimerProps) => {
  ...
  // rAF loop while active, just to repaint mm:ss.
  useEffect(() => {
  ...
  // Hourglass: top half empties, bottom fills based on elapsed ratio.
  ...
  return (
    ...
    >
      ...
      <div className="relative flex h-7 w-7 items-center justify-center">
        ...
        />
        {/* Sand level indicator: a bottom bar that fills as time elapses */}
        <span
          aria-hidden
          className={cn(
